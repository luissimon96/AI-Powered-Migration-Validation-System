...
class InputValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        try:
            ...
            self.logger.warning("Input validation failed", detail=e.detail, status_code=e.status_code)
            raise
    ...
    def _prevent_path_traversal(self, path: str):
        ...
        if normalized_path.startswith("../") or normalized_path.startswith("/") and ".." in normalized_path:
            self.logger.warning("Normalized path traversal attempt detected", path=path, normalized_path=normalized_path)
            raise HTTPException(status_code=400, detail="Path traversal detected")
    ...
    async def _validate_file_uploads(self, request: Request):
        ...
        for field_name in form:
            ...
            if uploaded_files_count > self.max_files_per_request:
                self.logger.warning("Too many files uploaded", count=uploaded_files_count, limit=self.max_files_per_request)
                raise HTTPException(status_code=400, detail=f"Maximum {self.max_files_per_request} files allowed per request")

            file_content = await file.read()
            if len(file_content) > self.max_file_size:
                self.logger.warning("File size limit exceeded", filename=file.filename, size=len(file_content), limit=self.max_file_size)
                raise HTTPException(status_code=400, detail=f"File {file.filename} too large. Max size: {self.max_file_size / (1024 * 1024):.1f} MB")
            ...
            if not detected_mime_type:
                ...
                raise HTTPException(status_code=400, detail=f"File {file.filename} content type not allowed or recognized")
    ...
    def _validate_file_content(self, content: bytes, filename: str) -> Optional[str]:
        ...
        for signature, mime_type in self.FILE_SIGNATURES.items():
            if content.startswith(signature):
                self.logger.info("File content validated by magic bytes", filename=filename, mime_type=mime_type)
                return mime_type
    ...
    def _scan_for_malware(self, content: bytes, filename: str):
        ...
        self.logger.info("File sent for malware scan (placeholder)", filename=filename, size=len(content))
