...
from ..analyzers import CodeAnalyzer, VisualAnalyzer, BaseAnalyzer
...
from ..services.llm_service import create_llm_service, LLMService, LLMServiceError
...
class MigrationValidator:
    ...
    async def validate_migration(
        ...
        try:
            ...
            session.add_log(f"Source analysis complete: {len(session.source_representation.ui_elements)} UI elements, "
            ...
            session.add_log(f"Target analysis complete: {len(session.target_representation.ui_elements)} UI elements, "
            ...
            session.add_log(f"Validation complete: {session.result.overall_status} "
    ...
    def get_supported_technologies(self) -> Dict[str, Any]:
        ...
        return {
            "technologies": [tech.value for tech in TechnologyContext.__annotations__["type"].__args__],
            "validation_scopes": [scope.value for scope in ValidationScope],
            "input_types": [input_type.value for input_type in InputType],
            "capabilities": {
                "code_analysis": [
                    "python", "javascript", "typescript", "java", "csharp", "php", "html"
                ],
                "visual_analysis": [
                    "screenshots", "ui_mockups", "interface_designs"
                ],
                "comparison_features": [
                    "semantic_matching", "fuzzy_element_detection", "business_logic_analysis"
                ],
    ...
    async def validate_request(self, request: MigrationValidationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if not source_analyzer.supports_scope(request.validation_scope):
                issues.append(f"Source technology {request.source_technology.type.value} "
        ...
        try:
            ...
            if not target_analyzer.supports_scope(request.validation_scope):
                issues.append(f"Target technology {request.target_technology.type.value} "
                            f"doesn't support validation scope {request.validation_scope.value}")
        except AnalyzerError as e:
        ...
        total_screenshots = len(request.source_input.screenshots) + len(request.target_input.screenshots)
        
        if total_files > 50:
            warnings.append(f"Large number of files ({total_files}) may impact processing time")
        
        if total_screenshots > 10:
            warnings.append(f"Large number of screenshots ({total_screenshots}) may impact processing time")
