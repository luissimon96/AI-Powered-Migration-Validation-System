...
class InputProcessor:
    ...
    def create_validation_request(
        ...
        try:
            ...
            raise ValueError(f"Unsupported source technology: {source_technology}. "
                           f"Supported: {[t.value for t in TechnologyType]}")
        
        try:
            target_tech_type = TechnologyType(target_technology)
        except ValueError:
            raise ValueError(f"Unsupported target technology: {target_technology}. "
                           f"Supported: {[t.value for t in TechnologyType]}")
    ...
    def _process_input_data(
        ...
        for file_path in files:
            ...
            if file_size > self.max_file_size:
                raise ValueError(f"{context.title()} file too large: {file_path} "
                               f"({file_size} bytes, max {self.max_file_size})")
            
            # Check file extension
            _, ext = os.path.splitext(file_path.lower())
            if ext not in self.allowed_code_extensions:
                raise ValueError(f"{context.title()} file has unsupported extension: {file_path} "
        ...
        for screenshot_path in screenshots:
            ...
            if not os.path.isfile(screenshot_path):
                raise ValueError(f"{context.title()} screenshot path is not a file: {screenshot_path}")
            
            # Check file size
            file_size = os.path.getsize(screenshot_path)
            if file_size > self.max_file_size:
                raise ValueError(f"{context.title()} screenshot too large: {screenshot_path} "
                               f"({file_size} bytes, max {self.max_file_size})")
            
            # Check file extension
            _, ext = os.path.splitext(screenshot_path.lower())
            if ext not in self.allowed_image_extensions:
                raise ValueError(f"{context.title()} screenshot has unsupported extension: {screenshot_path} "
        ...
        if total_size > self.max_total_size:
            raise ValueError(f"{context.title()} total file size too large: {total_size} bytes "
    ...
    def get_technology_options(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "input_types": [
                {"value": input_type.value, "label": self._format_input_type_label(input_type.value)}
    ...
    def validate_technology_compatibility(
        ...
        frontend_techs = ["javascript-react", "javascript-vue", "javascript-angular", 
                         "typescript-react", "typescript-vue", "typescript-angular"]
        backend_techs = ["python-flask", "python-django", "java-spring", "csharp-dotnet", "php-laravel"]
