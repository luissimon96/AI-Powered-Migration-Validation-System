...
from ..services.llm_service import LLMService, create_llm_service, LLMServiceError
...
class SemanticComparator:
    ...
    async def compare(
        ...
        weights = self.comparison_weights.get(scope, self.comparison_weights[ValidationScope.FULL_SYSTEM])
        
        # Compare different aspects based on scope weights
        if weights['ui_elements'] > 0:
            ui_discrepancies = await self._compare_ui_elements(source.ui_elements, target.ui_elements)
            discrepancies.extend(ui_discrepancies)
        
        if weights['data_fields'] > 0:
            data_discrepancies = await self._compare_data_fields(source.data_fields, target.data_fields)
    ...
    async def _compare_ui_elements(
        self,
        source_elements: List[UIElement],
    ...
    def _find_fuzzy_ui_match(self, source_elem: UIElement, target_elements: List[UIElement]) -> Optional[UIElement]:
        ...
        for target_elem in target_elements:
            if (source_elem.type == target_elem.type and
                source_elem.text and target_elem.text and
                self._text_similarity(source_elem.text, target_elem.text) > 0.8):
                return target_elem
    ...
    def _compare_ui_element_attributes(
        ...
        if (source_elem.position and target_elem.position and
            source_elem.position != target_elem.position):
            discrepancy = ValidationDiscrepancy(
    ...
    async def _compare_data_fields(
        ...
        for name, target_field in target_map.items():
            if name not in source_map and not self._is_likely_field_rename(target_field, source_fields):
                discrepancy = ValidationDiscrepancy(
        ...
        for name in source_map.keys() & target_map.keys():
            ...
            if source_field.required != target_field.required:
                severity = SeverityLevel.CRITICAL if source_field.required and not target_field.required else SeverityLevel.WARNING
                discrepancy = ValidationDiscrepancy(
    ...
    def _find_fuzzy_field_match(self, source_field: DataField, target_fields: List[DataField]) -> Optional[DataField]:
        """Find potential fuzzy match for a data field."""
        # Look for fields with similar names and same type
        for target_field in target_fields:
            if (source_field.type == target_field.type and
                self._field_name_similarity(source_field.name, target_field.name) > 0.7):
                return target_field
        return None
    ...
    def _is_likely_field_rename(self, target_field: DataField, source_fields: List[DataField]) -> bool:
    ...
    def _find_fuzzy_function_match(self, source_func: BackendFunction, target_functions: List[BackendFunction]) -> Optional[BackendFunction]:
        ...
        for target_func in target_functions:
            if (self._function_name_similarity(source_func.name, target_func.name) > 0.7 and
                self._parameter_similarity(source_func.parameters, target_func.parameters) > 0.8):
                return target_func
        return None
    ...
    def _compare_function_details(
        self,
        source_func: BackendFunction,
        target_func: BackendFunction
    ) -> List[ValidationDiscrepancy]:
        ...
        if (source_func.logic_summary and target_func.logic_summary and
            source_func.logic_summary != target_func.logic_summary):
            discrepancy = ValidationDiscrepancy(
                type="logic_change",
                severity=SeverityLevel.WARNING,
