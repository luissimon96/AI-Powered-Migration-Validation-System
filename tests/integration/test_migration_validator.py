...
class TestMigrationValidatorIntegration:
    ...
    async def test_complete_validation_pipeline(self, sample_validation_request, mock_llm_service):
        ...
        assert session.result.overall_status in ["approved", "approved_with_warnings", "rejected"]
    ...
    async def test_validation_with_different_scopes(self, sample_validation_request, mock_llm_service):
...
class TestMigrationValidatorConfiguration:
    ...
    def test_validator_initialization_with_config(self, mock_get_config, mock_llm_service):
        ...
        from src.core.config import ValidationConfig, LLMProviderConfig
